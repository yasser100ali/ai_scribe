...
def stream_patient_text_with_audio(messages: List[dict], protocol: str = "data"):
    """
    Handle audio output for patient chat using Chat Completions API with audio modalities.
    ...
    try:
        # Use Chat Completions API with audio modalities - use a more empathetic voice
        response = client.chat.completions.create(
            model="gpt-4o-audio-preview",
            modalities=["text", "audio"],
            audio={"voice": "nova", "format": "wav"},  # Nova is more warm and empathetic for patients
